...
export class MetadataViewComponent
  extends AbstractViewComponent<Metadata>
  implements OnInit
{
  ...
  postFind() {
    ...
    this.fieldDefinitionService.getAllList().subscribe((fieldDefinitions) => {
      this.fieldDefinitions = fieldDefinitions.sort(
      ...
      .subscribe(
        (fieldDefinitions) => {
          if (!fieldDefinitions || fieldDefinitions.length === 0) {
            this.addError(
              "Unable to create metadata: at least one field definition is required.",
            );
            return;
          }

          if (this.element.table_key_type === "slug") {
            const tableKeyAddition = (
              this.element.table_key_addition ?? ""
            ).trim();
            if (!tableKeyAddition) {
              this.addError(
                "Unable to create metadata: Table key Addition is required when table key type is slug.",
              );
              return;
            }

            const existsMatchingField = fieldDefinitions.some(
              (fd) => fd.name === tableKeyAddition,
            );
            if (!existsMatchingField) {
              this.addError(
                "Unable to create metadata: Table key Addition must match the name of an existing field definition.",
              );
              return;
            }
          }
  ...
  public editField(fieldDefinition: FieldDefinition) {
    ...
    this.router.navigate(
      ["/fielddefinition/edit", fieldDefinition.uuid],
      navigationExtras,
    );
  }
